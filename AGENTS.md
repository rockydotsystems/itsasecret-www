...
- **No MFA in v1** - master-password auth first; TOTP/passkeys later. (The recovery phrase below is device trust, not MFA.)
- **Recovery phrase = device-trust second factor, not key material** - a 30-word phrase generated at register (EFF wordlist, `src/lib/recovery-phrase.ts`), hashed into `users.recovery_phrase_hash` with its own Argon2id params, and shown exactly once in the register response. Login from an untrusted machine fails with `403 code=RECOVERY_REQUIRED` until the client runs `POST /api/auth/verify-recovery {email, recoveryPhrase, devicePubkey}` and receives a device token (hashed into `device_tokens`, 1-year absolute expiry; trust also lapses after 90 idle days). Trusted devices then skip the phrase by sending `deviceToken` on login. Rotation is authenticated-only via `POST /api/auth/regenerate-recovery` and burns the old phrase immediately; optional `revokeDevices` / `revokeOtherSessions` mirror the profile-page checkboxes. Legacy accounts (`recovery_phrase_hash IS NULL`) are gated into forced setup by `requireAuth` (opt out per-route with `{ allowNoRecovery: true }`) and the `/setup-recovery` page. The phrase never touches key material - org keys stay wrapped under the master password only.
